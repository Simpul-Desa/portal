...
const KELAS_BUTIR = "relative flex size-9.5 md:size-11 shrink-0 items-center justify-center rounded-full transition-all duration-200 shadow-xs md:shadow-sm";
 ...
 *
 * Pada seluler (<768px): Rail berbaring rapi di bawah sebagai dock navigasi melayang
 * yang terangkat (`bg-float/95 backdrop-blur-md rounded-2xl shadow-float border border-line/60`),
 * mendistribusikan item secara merata dan mudah dijangkau ibu jari tanpa terpotong.
 */
export function SideRail(props: SideRailProps) {
  ...
  return (
    <nav
      ...
      className="z-30 order-last flex h-14 w-full shrink-0 flex-row items-center justify-between rounded-2xl bg-float/95 backdrop-blur-md px-2.5 py-1.5 shadow-float border border-line/60 md:order-none md:h-auto md:w-20 md:flex-col md:justify-start md:gap-4 md:rounded-card md:bg-transparent md:border-0 md:shadow-none md:px-0 md:py-4 overflow-x-auto md:overflow-x-visible [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
    >
      {diAdmin ? (
        <div className="group relative flex shrink-0 items-center justify-center hover:z-[100]">
          <Link
            href="/"
            aria-label="Simpul Desa — kembali ke dasbor"
            className={`flex items-center justify-center p-1 md:p-2 cursor-pointer transition-colors ${FOCUS_RING}`}
          >
            <LogoSimpul />
          </Link>
      ...
      <div className="flex flex-1 flex-row items-center justify-around gap-1 md:gap-4 md:flex-initial md:flex-col md:mt-6">
        {LENSA_ITEMS.map(({ nama, icon: Icon, lensa }) => {
          ...
          const kelas = `${KELAS_BUTIR} ${FOCUS_RING} cursor-pointer ${
            aktif
              ? "bg-primary text-white"
              : "bg-float text-ink hover:bg-ink hover:text-canvas"
          }`;
          ...
          if (diAdmin) {
            return (
              <div key={lensa} className="group relative flex shrink-0 items-center justify-center hover:z-[100]">
                <Link
                  href={`/?lensa=${lensa}`}
                  aria-label={labelTombol}
                  className={kelas}
                >
                  <Icon className="size-[17px]! md:size-5!" />
                  {lencana}
                </Link>
                <div className="pointer-events-none absolute left-full top-1/2 ml-4 -translate-y-1/2 rounded bg-ink px-2.5 py-1.5 text-xs text-canvas opacity-0 shadow-float transition-opacity group-hover:opacity-100 hidden md:block whitespace-nowrap z-[100]">
                  {judulTombol}
      ...
      <span className="h-5 w-px shrink-0 bg-line/60 mx-1 md:mx-0 md:h-px md:w-6" />
      ...
      {!memuat && bisaAdmin && (
        <div className="group relative flex shrink-0 items-center justify-center hover:z-[100]">
          ...
          >
            <Shield className="size-4.5 md:size-5" strokeWidth={1.5} aria-hidden="true" />
          </Link>
          <div className="pointer-events-none absolute left-full top-1/2 ml-4 -translate-y-1/2 rounded bg-ink px-2.5 py-1.5 text-xs text-canvas opacity-0 shadow-float transition-opacity group-hover:opacity-100 hidden md:block whitespace-nowrap z-[100]">
            Halaman Admin
          </div>
      ...
      <div className="group relative flex shrink-0 items-center justify-center hover:z-[100] md:mt-auto">
        <MenuAkun>
          <button
            type="button"
            aria-label="Akun"
