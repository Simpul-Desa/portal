...
export function AdminSidebar({ tab, onTab, idPanel, idTab, emailAdmin }: TabBarProps) {
  ...
  return (
    <aside className="flex w-full shrink-0 flex-col justify-between border-b border-hairline bg-surface/35 p-2 md:w-64 md:border-b-0 md:border-r md:p-4 lg:w-72">
      <div>
        <div className="mb-1.5 px-2 md:px-3 pt-1">
        ...
        <div
          ...
          className="flex flex-row gap-1.5 overflow-x-auto md:flex-col md:overflow-x-visible pb-1 md:pb-0 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
        >
          {TAB_ADMIN.map((t) => {
            ...
            return (
              <button
                ...
                tabIndex={aktif ? 0 : -1}
                onClick={() => onTab(t.id)}
                className={`group flex flex-1 cursor-pointer items-center gap-2 md:gap-3 rounded-xl px-2.5 py-2 md:px-3.5 md:py-3 text-left transition-all md:flex-none ${FOCUS_RING} ${
                  aktif
              ...
              >
                <div
                  className={`flex size-7 md:size-8 shrink-0 items-center justify-center rounded-lg transition-colors ${
                    aktif
                      ? "bg-primary/10 text-primary"
                      : "bg-surface text-muted group-hover:bg-surface group-hover:text-ink"
                  }`}
                >
                  <Icon size={16} className="md:w-[18px] md:h-[18px]" strokeWidth={aktif ? 2 : 1.75} />
                </div>
